...
let AnalyticsService = class AnalyticsService {
    ...
    async getProjectAnalytics() {
        ...
        const projectAnalytics = projects.map((project) => {
            ...
            const completedWorkPackages = project.workPackages.filter((wp) => wp.status === 'COMPLETED').length;
            ...
            return {
                ...
                completionPercentage: totalWorkPackages > 0
        ...
        return {
            ...
            summary: {
                ...
                totalWorkPackageAmount: projectAnalytics.reduce((sum, p) => sum + p.financial.totalWorkPackageAmount, 0),
                totalPaid: projectAnalytics.reduce((sum, p) => sum + p.financial.totalPaid, 0),
                totalPending: projectAnalytics.reduce((sum, p) => sum + p.financial.pending, 0),
            },
        };
    }
    async getFinancialAnalytics(startDate, endDate) {
        ...
        const paymentsByAccount = payments.reduce((acc, payment) => {
        ...
        return {
            ...
            totalAccountBalance: accountBalances.reduce((sum, acc) => sum + Number(acc.currentBalance), 0),
        };
    }
    async getWorkPackageAnalytics() {
        ...
        const statusDistribution = workPackages.reduce((acc, wp) => {
        ...
        const inProgressWorkPackages = workPackages.filter((wp) => wp.status === 'IN_PROGRESS');
        return {
            ...
            completion: {
                ...
                completionRate: workPackages.length > 0
                    ? ((completedWorkPackages.length / workPackages.length) * 100).toFixed(2)
                    : 0,
            },
            workPackagesByProject: workPackages.reduce((acc, wp) => {
